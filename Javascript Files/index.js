...
//ADICIONAR A FUNCIONALIDADE DO CARRINHO GERANDO PROBLEMAS
/*
...
for (var i = 0; i < addCart.length; i++) {
	addCart[i].addEventListener('click', (event) => {
		...
		if (cartItems.innerHTML.includes(`<div class="itemCart bg-ligh">
			...
			//AUMENTAR A QUANTIDADE DE ITEM NO CARRINHO
...
placeOrder.addEventListener('click', (evv) => {
	...
	if (accessInnerHtml.innerHTML === '') {
		alert('Prezado cliente, você pode adicionar produtos ao carrinho antes de fazer qualquer pedido')
	} else {
		alert('Pedido realizado com sucesso.Custo total do pedido feito = $' + getPrice + '.' + 'Obrigado por comprar conosco');
